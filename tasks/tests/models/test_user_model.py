...
class UserModelTestCase(TestCase):
    ...
    def test_first_name_must_not_contain_more_than_50_characters(self):
        self.user.first_name = "x" * 60
        self._assert_user_is_invalid()

#error
    def test_user_get_tasks_is_valid(self):
        string = "\n".join([p.tasks for p in self.user.tasks.all()])
        self.assertEqual(self.user.get_tasks(self.user), string)
